...
* **Memory + Recs** (exploring) — a personal/team second-brain that proactively surfaces what you should see next, not just retrieves what you stored. Ingests reading history, tweets, conversations, bookmarks; learns from interactions; recommends new content + relevant past memories. Differentiation vs Mem/Reflect/mymind: those retrieve, this recommends. Moat: ingestion breadth × interaction history × pluggable weights. The original "pluggable rec engine as a service" is the underlying primitive — productize the consumer surface first.
